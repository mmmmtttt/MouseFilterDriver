...
#define INITGUID
#include "guiddef.h"
DEFINE_GUID(GUID_MOU_CLASS, 0x4d36e96fL, 0xe325, 0x11ce, 0xbf, 0xc1, 0x08, 0x00, 0x2b, 0xe1, 0x03, 0x18);
...
DEFINE_GUID(GUID_DEVINTERFACE_MOUCLASS, 0x4d36e96fL, 0xe325, 0x11ce, 0xbf, 0xc1, 0x08, 0x00, 0x2b, 0xe1, 0x03, 0x18);
...
int __cdecl _tmain(int argc, _In_reads_(argc) LPTSTR argv[])
{
    ...
    static const GUID* deviceGuid = &GUID_MOU_CLASS;
...
#pragma prefast(suppress:6385, "Previously checked argIndex being less than argc. No buffer overflow.")
    ...
    devInfo = SetupDiGetClassDevs(deviceGuid,
        NULL,
        NULL,
        DIGCF_PROFILE);
    ...
    for (deviceIndex = 0;
        ...
        // there was a match.
        if (deviceHWId != NULL &&
            !DeviceHWIdMatches(devInfo, &devInfoData, deviceHWId)
            ) {
...
)
{
    ...
    // set the new list of upperfilters
    ...
    if (!SetupDiSetDeviceRegistryProperty(DeviceInfoSet,
        ...
        (LPBYTE)characteristic,
        (lstrlen(characteristic) + 1 + 1) * sizeof(TCHAR)) * 2
        )
    {
        printf("in AddUpperFilterDriver(): "
            "couldn't set registry value! error: %u\n", GetLastError());
...
)
{
    ...
    if (deviceHWId != NULL)
    {
        ...
        {
            printf("in DeviceHWIdMatches(): registry key is not an SZ!\n");
            matching = FALSE;
        }
        else
        {
            // do the strings match?
            matching = (_tcscmp(deviceHWId, DeviceHWId) == 0) ? TRUE : FALSE;
        }
        free(deviceHWId);
    }
    else
    {
        printf("in DeviceHWIdMatches(): registry key is NULL!\n");
        matching = FALSE;
    }
